        ...
        {selectedCards.length === 0 ? (
          <div className="border border-dashed border-gray-200 bg-gray-50/50 rounded-lg py-10 px-4 text-center">
            <p className="text-sm text-secondary">
              Выберите карты выше, чтобы сравнить условия
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            {selectedCards.map((card) => (
              <div
                key={card.id}
                className="bg-white border border-red-600 rounded-lg shadow-sm p-4 relative"
              >
                <button
                ...
                <dl className="divide-y divide-border-subtle">
                  {FEATURE_ROWS.map(({ key, label }) => {
                    ...
                    return (
                      ...
                      >
                        <dt
                          className={`text-xs font-medium flex-shrink-0 ${isDiff ? 'text-red-600 font-semibold' : 'text-secondary'}`}
                        >
                          {label}
                        </dt>
                        <dd
                          className={`text-xs text-right font-semibold ${isDiff ? 'text-red-600 font-bold' : ''}`}
                        >
                          {card.features[key]}
                        </dd>
                      </div>
            ...
            if (!card) {
              return (
                <div
                  ...
                  className="border border-dashed border-gray-200 bg-gray-50/50 rounded shadow-none min-h-[120px]"
                />
              );
            }
            return (
              <div
                key={card.id}
                className="bg-white p-3 sm:p-4 border border-red-600 relative group shadow-sm rounded"
              >
                <button
                  onClick={() => removeCard(card.id)}
        ...
        <div className="space-y-2">
          {FEATURE_ROWS.map(({ key, label }) => {
            ...
            return (
              <div
                key={key}
                className={`grid grid-cols-4 gap-3 sm:gap-4 p-3 sm:p-4 items-center border transition-all ${
                  isDiff
              ...
              >
                <div
                  className={`text-xs sm:text-sm ${isDiff ? 'text-red-600 font-semibold' : 'text-secondary font-medium'}`}
                >
                  {label}
                </div>
                {tableSlots.map((card, index) =>
                  card ? (
                    ...
                    <div
                      key={`empty-${key}-${index}`}
                      className="text-outline-variant italic text-xs sm:text-sm text-gray-300"
                    >
                      —
                    </div>
                  )
